...
async def send_to_external_api(session: UserSession) -> Dict[str, Any]:
    ...
    try:
        print(f"[API_CALL] Sending data to {EXTERNAL_API_URL}")
        print(f"[API_PAYLOAD] {json.dumps(api_payload, ensure_ascii=False)}")
        ...
        print(f"[API_RESPONSE] Status: {response.status_code}, Body: {response.text}")
